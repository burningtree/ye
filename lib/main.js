...
Document.prototype.changeElement = function() {
  ...
  var lastState = { index: '""', nested: cur.nested, type: 'key' };
  this.data = objChangeEl(this.data, cur, '""');
  this.insertMode(function(val) {
    if(!val) {
      _this.data = objChangeEl(_this.data, lastState, cur.index);
      _this.refresh();
      return;
    }
    ...
    lastState = { index: tval, nested: lastState.nested, type: 'key' }; 
    _this.refresh();
  });
};
...
Document.prototype.insertMode = function(cb, onChange) {
  var _this = this;
  var box = blessed.box({
    parent: this.ui,
    style: {
  ...
  input.on('keypress', function(ch) {
    ...
    if(ch == ':') {
      return callback(input.value.trim(':'));
    }
    onChange(input.value + ch);
    _this.screen.render();
  });
  input.key(['escape', 'enter'], function() {
    callback(input.value.trim());
  });

  input.focus();
  this.engine.echo('INSERT MODE');
  this.refresh();
};
