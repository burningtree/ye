...
Document.prototype.stringify = function(obj, startLine, parent, forceShow) {
  var _this = this;
  var out = [];
  var map = {};
  var line = startLine;
  if(type(obj) === 'object' || type(obj) === 'array') {
    ...
    Object.keys(obj).forEach(function(key, i) {
      ...
      valResult = _this.stringify(obj[key], cline, mapKey);
      oval = valResult.lines.join('\n');
      var header = padding + '{bold}' + okey + '{/bold}: ';
      if(valResult.nested) {
    ...
    return { lines: out, map: map, nested: true };
  }
  else {
  }
  return { lines: [ _this.engine.tools.yaml.dump(obj).trim() ], nested: false };
}
...
Document.prototype.content = function(selected) {
  ...
  var result = this.stringify(_.clone(this.data), 0);
